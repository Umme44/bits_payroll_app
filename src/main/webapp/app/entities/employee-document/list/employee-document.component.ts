...
import { EntityArrayResponseType, EmployeeDocumentService } from '../service/employee-document.service';
import { EmployeeDocumentDeleteDialogComponent } from '../delete/employee-document-delete-dialog.component';
import { ParseLinks } from 'app/core/util/parse-links.service';

@Component({
  selector: 'jhi-employee-document',
...
export class EmployeeDocumentComponent implements OnInit {
  ...
  delete(employeeDocument: IEmployeeDocument): void {
    const modalRef = this.modalService.open(EmployeeDocumentDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.employeeDocument = employeeDocument;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.employeeDocuments = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IEmployeeDocument[] | null): IEmployeeDocument[] {
    const employeeDocumentsNew = this.employeeDocuments ?? [];
    if (data) {
      for (const d of data) {
        if (employeeDocumentsNew.map(op => op.id).indexOf(d.id) === -1) {
          employeeDocumentsNew.push(d);
        }
      }
    }
    return employeeDocumentsNew;
  }
