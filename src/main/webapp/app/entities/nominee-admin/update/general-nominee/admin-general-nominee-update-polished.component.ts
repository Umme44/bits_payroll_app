...
@Component({
  selector: 'jhi-admin-general-nominee-update-polished',
  templateUrl: './admin-general-nominee-update-polished.component.html',
})
export class AdminGeneralNomineeUpdatePolishedComponent implements OnInit, OnDestroy {
  isSaving = false;
  employees: IEmployee[] = [];
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ nominee }) => {
      ...
      if (nominee.id) {
        ...
        this.routerNominee = nominee;
        this.selectedEmployeeId = this.routerNominee.employeeId!;
  ...
  onClickRedirectToViewPage(nominee: INominee): void {
    if (nominee.id) {
      this.router.navigate(['/nominee/general/' + nominee.id + '/view']);
    }
  }
