...
@Component({
  selector: 'jhi-employee-resignation-approve',
  templateUrl: './employee-resignation-approve.component.html',
})
export class EmployeeResignationApproveComponent implements OnInit {
  isSaving = false;
  employees: IEmployee[] = [];
  createdAtDp: any;
  updatedAtDp: any;
  ...
  save(): void {
    ...
    if (employeeResignation.id !== undefined) {
      this.subscribeToSaveResponse(this.employeeResignationService.approve(employeeResignation));
    } else {
      // this.subscribeToSaveResponse(this.employeeResignationService.create(employeeResignation));
    }
  }
  ...
  protected onSaveSuccess(): void {
    Swal.fire({
      icon: 'success',
      html: 'Approved! <br/> Populating data...',
