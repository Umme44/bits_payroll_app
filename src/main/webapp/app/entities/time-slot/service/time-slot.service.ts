...
type RestOf<T extends ITimeSlot | NewTimeSlot> = Omit<T, 'inTime' | 'outTime'> & {
  inTime?: string | null;
  outTime?: string | null;
};
...
export class TimeSlotService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/attendance-mgt/time-slots');
  protected commonResourceUrl = this.applicationConfigService.getEndpointFor('api/common/time-slots');
  ...
  update(timeSlot: ITimeSlot): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(timeSlot);
    return this.http
      .put<RestTimeSlot>(`${this.resourceUrl}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  findAll(): Observable<EntityArrayResponseType> {
    return this.http
      .get<RestTimeSlot[]>(this.resourceUrl + '/all', { observe: 'response' })
      .pipe(map(res => this.convertResponseArrayFromServer(res)));
  }
  ...
  getAllTimeSlots(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<RestTimeSlot[]>(this.commonResourceUrl, { params: options, observe: 'response' })
      .pipe(map(res => this.convertResponseArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient<T extends ITimeSlot | NewTimeSlot | PartialUpdateTimeSlot>(timeSlot: T): RestOf<T> {
    return {
      ...timeSlot,
      inTime: timeSlot.inTime?.toJSON() ?? null,
      outTime: timeSlot.outTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTimeSlot: RestTimeSlot): ITimeSlot {
    return {
      ...restTimeSlot,
      inTime: restTimeSlot.inTime ? dayjs(restTimeSlot.inTime) : undefined,
      outTime: restTimeSlot.outTime ? dayjs(restTimeSlot.outTime) : undefined,
    };
  }
