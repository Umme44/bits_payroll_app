...
export class EmployeeNocAdminService {
  ...
  deleteEmployeeNoc(id: number): Observable<HttpResponse<{}>> {
    return this.http.delete(`${this.resourceUrlForEmployeeNoc}/${id}`, { observe: 'response' });
  }

  findEmployeeNocPrintFormatById(id: number): Observable<EmployeeNocEntityResponseType> {
    return this.http
      .get<IEmployeeNOC>(`${this.resourceUrlForEmployeeNoc}/print-format/${id}`, { observe: 'response' })
      .pipe(map((res: EmployeeNocEntityResponseType) => this.convertDateFromServer(res)));
  }

  approveEmployeeNoc(approvalDto: CertificateApprovalDto, id: number): Observable<EmployeeNocEntityResponseType> {
    ...
    return this.http
      .post<CertificateApprovalDto>(this.resourceUrlForEmployeeNoc + `/approve/${id}`, copy, { observe: 'response' })
      .pipe(map((res: EmployeeNocEntityResponseType) => this.convertDateFromServer(res)));
  }

  // rejectEmployeeNoc(approvalDto: CertificateApprovalDto, id: number): Observable<EmployeeNocEntityResponseType> {
  //   const copy = Object.assign({}, approvalDto);
  //   return this.http
  //     .post<CertificateApprovalDto>(this.resourceUrlForEmployeeNoc + `/reject/${id}`, copy, { observe: 'response' })
  //     .pipe(map((res: EmployeeNocEntityResponseType) => this.convertDateFromServer(res)));
  // }
  ...
  isReferenceNumberUnique(referenceNumber: string): Observable<HttpResponse<boolean>> {
    ...
    return this.http.get<boolean>(this.resourceUrlForEmployeeNoc + `/is-unique`, { params: options, observe: 'response' });
  }
