...
export class IncrementListComponent implements OnInit, OnDestroy {
  ...
  delete(employmentHistory: IEmploymentHistory): void {
    swalOnDeleteConfirmation().then(result => {
      if (result.isConfirmed) {
        this.employmentActionsService.deleteIncrement(employmentHistory.id!).subscribe(
          res => {
            swalOnDeleteSuccess();
  ...
  protected onSuccess(data: IEmploymentHistory[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    ...
    this.page = page;
    if (navigate) {
      this.router.navigate(['/employment-actions/increment'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
