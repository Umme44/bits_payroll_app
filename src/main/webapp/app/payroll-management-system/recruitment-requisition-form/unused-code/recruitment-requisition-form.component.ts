...
export class RecruitmentRequisitionFormComponent implements OnInit, OnDestroy {
  ...
  registerChangeInRecruitmentRequisitionForms(): void {
    this.eventSubscriber = this.eventManager.subscribe('recruitmentRequisitionFormListModification', () => this.loadPage());
  }

  delete(recruitmentRequisitionForm: IRecruitmentRequisitionForm): void {
    const modalRef = this.modalService.open(RecruitmentRequisitionFormDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.recruitmentRequisitionForm = recruitmentRequisitionForm;
  }
