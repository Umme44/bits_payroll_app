...
import { UpcomingEventContractEndService } from './upcoming-event-contract-end.service';
...
@Component({
  selector: 'jhi-upcoming-event-contract-end',
  templateUrl: './upcoming-event-contract-end.component.html',
  styleUrls: ['upcoming-event-contract-end.component.scss'],
})
export class UpcomingEventContractEndComponent implements OnInit, OnDestroy {
  employees?: IEmployee[];
  isLoading = false;
  ...
  constructor(
    ...
    private upcomingEventContractEndService: UpcomingEventContractEndService,
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.upcomingEventContractEndService
      .query(this.filters, {
        page: pageToLoad - 1,
  ...
  protected onSuccess(data: IEmployee[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    ...
    this.page = page;
    if (navigate) {
      this.router.navigate(['/upcoming-event-contract-end'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
  ...
  exportFiltered(): void {
    // todo: need fix
    // this.exportService.exportExcel(this.employees!, 'FilteredEmployees');
  }

  exportAll(): void {
    // todo: need fix
    // this.employeeService.query().subscribe((res: HttpResponse<IEmployee[]>) => (this.allemployees = res.body || []));
    // this.exportService.exportExcel(this.allemployees!, 'AllEmployees');
    // this.allemployees = [];
  }
