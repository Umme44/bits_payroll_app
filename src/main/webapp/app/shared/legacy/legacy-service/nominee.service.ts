...
import { IPfNomineeEmployeeDetailsDTO } from '../../model/pf-nominee-employee-details.model';
import { IEmployeeNomineeInfo } from '../../model/employee-nominee-info.model';
import { createRequestOption } from '../../../core/request/request-util';
import { INomineeMaster } from '../../model/nominee-master.model';
import { DATE_FORMAT } from '../../../config/input.constants';
import dayjs from 'dayjs/esm';
import { IEmployee } from '../legacy-model/employee.model';
...
export class NomineeService {
  ...
  getEmployeesWithNominees(req?: any): Observable<EmployeeNomineeInfoArrayResponseType> {
    ...
    const options = createRequestOption(req);
    return this.http
      .get<IEmployeeNomineeInfo[]>(this.resourceUrl + '/employees-with-nominees', {
        params: options,
        observe: 'response',
      })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServerV2(res)));
  }

  getNomineesByEmployeePin(pin: string): Observable<INomineeMaster> {
    return this.http.get<INomineeMaster>(this.resourceUrl + '/all/' + pin);
  }
  ...
  protected convertDateArrayFromServerV2(res: EmployeeNomineeInfoArrayResponseType): EntityArrayResponseType {
    if (res.body) {
