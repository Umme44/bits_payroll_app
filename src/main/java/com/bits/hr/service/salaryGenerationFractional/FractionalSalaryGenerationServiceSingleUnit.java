...
@Service
@Log4j2
public class FractionalSalaryGenerationServiceSingleUnit {
    ...
    // new method
    public Optional<EmployeeSalary> generateMonthlySalary(Employee employee, int year, int month, LocalDate startDate, LocalDate endDate) {
        try {
            ...
            if (employee.getMainGrossSalary() == null || employee.getMainGrossSalary() == 0d) {
            ...
            salary.setIsVisibleToEmployee(false);
            ...
            List<EmploymentHistory> employmentHistoryListSalaryChange = employmentHistoryList
                .stream()
                .filter(employmentHistory ->
                    !employmentHistory.getCurrentMainGrossSalary().equals(employmentHistory.getPreviousMainGrossSalary())
                )
                .collect(Collectors.toList());
            ...
            if (employmentHistoryList.isEmpty()) {
                ...
                List<Fraction> fractionsSet = fractionService.getFractions(
                    employee,
                    startDate,
                    endDate,
                    new ArrayList<>(employmentHistoryListSalaryChange)
                );
         ...
         */
            ...
            if (
                ...
                /*
                 * CR:(25-09-23) according to palash da,
                 * gratuity fund will be calculated from basic salary
                 * without limiting 10 service year or maximum 15 gratuity.
                 * because employee go through salary revisions every year.
                 */
                gratuityFund = MathRoundUtil.round((1.5d * mBasic) / 12);
                //old service --  gratuityFundService.getProvisionForGratuityFundPerMonth(employee.getDateOfJoining(), calculationDate, mBasic));
            }
            ...
            if (
                employee.getDesignation() != null && !employee.getDesignation().getDesignationName().trim().toLowerCase().equals("intern")
            ) {
            ...
            // salary.setProvisionForProjectBonus(provisionForProjectBonus);
            ...
            salary.setIsHold(salaryHoldService.isSalaryHold(employee.getId(), year, month));
            ...
            if (employee.getIsTaxPaidByOrganisation() != null && employee.getIsTaxPaidByOrganisation()) {
            ...
            String remarks = "";
            if (salary.getIsHold()) remarks += " Hold, FNF \n";
            if (pfLoanRepayment != 0d) remarks += " Deduction for PF Loan \n";
            if (DateUtil.isBetween(employee.getDateOfJoining(), startDate.minusDays(1), endDate.plusDays(1))) remarks += " New Joiner \n";
            ...
            customExecutorService.executorService.submit(() -> {
                pfCollectionService.createOrUpdatePfCollections(
                    salary,
                    employee,
    ...
    private double getArrearPf(Employee employee, int year, Month month) {
        List<PfArrear> arrearSalaryList = pfArrearRepository.findByEmployeeIdAndYearAndMonth(employee.getId(), year, month);
        return arrearSalaryList.stream().mapToDouble(PfArrear::getAmount).sum();
    }
    ...
    private void publishEvent(PfLoanRepaymentDTO pfLoanRepaymentDTO, com.bits.hr.service.event.EventType event) {
        log.debug("publishing leave application event with event: " + event);
        PFLoanRepaymentEvent pfLoanRepaymentEvent = new PFLoanRepaymentEvent(this, pfLoanRepaymentDTO, event);
        applicationEventPublisher.publishEvent(pfLoanRepaymentEvent);
    }
}
